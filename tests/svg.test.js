...
test("should render a 300x200 svg element", () => {
    const expectedSVG = `<svg version="1.1" width="300" height="200" xmlns="http://www.w3.org/2000/svg></sgv>`;
    const svg = new SVG();
...
test("should throw error if text exceeds 3 characters", () => {
    ...
    const svg = new SVG();
    const actualResult = () => svg.setText("HELLOO", "#333")

    expect(actualResult).toThrow(expectedSVG);
})

test("should include a shape", () => {
    const expectedSVG = `<svg version="1.1" width="300" height="200" xmlns="http://www.w3.org/2000/svg><rect x="90" y="40" width="120" height="120" fill="yellow"/><text x="150" y="125" font-size="60" text-anchor="middle" fill="red">A</text></svg>`;
    const svg = new SVG();
    svg.setText("SVG", "orange")
